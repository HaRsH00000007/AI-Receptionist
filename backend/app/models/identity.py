...
class User(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    There is no password column, and that is the design rather than an omission.
    Authentication is passwordless (magic link, and OAuth later), so the system
    never stores a credential that can be stolen, reused across sites, or leaked
    in a dump. What would be the password column's attack surface becomes a
    short-lived token in :class:`app.models.identity.Session` instead.

    ``email`` is stored lower-cased and is globally unique: it is the identifier
    a magic link is sent to, so two rows differing only in case would be two
    accounts for one inbox.
    """
